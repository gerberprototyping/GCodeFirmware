...
TIM_HandleTypeDef* StepDriver::timer;
...
void StepDriver::initAll(TIM_HandleTypeDef* timer) {
    LimitSwitch::initAll();
    Stepper::initAll();
    // timerInterrupt.init(STEP_INTERRUPT_PERIOD, &StepDriver::interruptHandler);
    StepDriver::timer = timer;
    timer->Init.Period = STEP_INTERRUPT_PERIOD;
    if (HAL_OK != HAL_TIM_Base_Init(timer)) {
        Error_Handler();
    }
    if (HAL_OK != HAL_TIM_RegisterCallback(timer, HAL_TIM_PERIOD_ELAPSED_CB_ID, &StepDriver::interruptHandler)) {
        Error_Handler();
    }
}
...
void StepDriver::start() {
    ...
    if (HAL_TIM_Base_Start_IT(StepDriver::timer) != HAL_OK) {
        Error_Handler();
    }
    running = 1;
}
...
void StepDriver::stop() {
    running = 0;
    // TimerInterrupt::stop();
    if (HAL_TIM_Base_Stop_IT(StepDriver::timer) != HAL_OK) {
        Error_Handler();
    }
}
...
void StepDriver::interruptHandler(TIM_HandleTypeDef* timer) {
    if (running) {
